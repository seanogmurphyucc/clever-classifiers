...
def removeAnnotations(proportion, train):
   
   train_copy = train.copy()

   if proportion > 0:      
      for index in train_copy.index:
        for a in annotations:
            if random.randrange(0,10) < proportion:
               train_copy.loc[a,index]= 0
   train = train_copy           
   print(train.head)
